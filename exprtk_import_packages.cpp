...
public:
   ...
   , symbol_table_(symbol_table)
   {
      ...
      package_map_.emplace("exprtk.rtl.io"     ,
                           [&](symbol_table_t& symbtab) -> bool { return symbtab.add_package(io_package    ); });

      package_map_.emplace("exprtk.rtl.io.file",
                           [&](symbol_table_t& symbtab) -> bool { return symbtab.add_package(file_package  ); });

      package_map_.emplace("exprtk.rtl.vecops" ,
                           [&](symbol_table_t& symbtab) -> bool { return symbtab.add_package(vector_package); });
   }

   inline T operator()(parameter_list_t parameters)
   {
      string_t package(parameters[0]);

      typename package_map_t::iterator itr = package_map_.find(exprtk::to_str(package));
